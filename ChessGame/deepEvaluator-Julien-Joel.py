...
class CustomNet(Module):
    def __init__(self):
        ...
        self.cnnModel = Sequential(                             # side =  (previous side - kernel size + stride + 2*padding)/stride
            # First layer
            Dropout(p=dropout),
            Conv2d(12, 8, kernel_size=3, stride=1), #  8-3 + 1  = 6
            ReLU(inplace=True),
            # Second layer
            Dropout(p=dropout),
            Conv2d(8, 16, kernel_size=3, stride=1), # 6 - 3 + 1   = 4
            ReLU(inplace=True),
            # Second layer
            Dropout(p=dropout),
            Conv2d(16, 32, kernel_size=3, stride=1), # 4 - 3 + 1  = 2
            ReLU(inplace=True),
            # Second layer
            Dropout(p=dropout),
            Conv2d(32, 64, kernel_size=2, stride=1), # 2 - 3 + 1 = 1
            ReLU(inplace=True),
